...
public class RouteMatching {
    ...
    public RouteMatching(HereConfiguration configuration) {
        this.configuration = configuration;
    }

    public RouteMatchModel calculateRouteMatching(TripModel trip) {
        String gpx = GpxFactory.getGpxAsString(trip);
        String response = httpHelper.doPost(String.format("matchroute.json?apiKey=%s&routemode=car&attributes=SPEED_LIMITS_FCn(FROM_REF_SPEED_LIMIT,TO_REF_SPEED_LIMIT)", configuration.apiKey), gpx);
        try {
